...
import { getMentee, updateMentee, updateRole, addRole, deleteRole, generateNarrative, addComment, updateComment, deleteComment } from '../utils/api'
...
export default function MentorMenteeView() {
  ...
  async function handleRoleUpdate(roleId, patch) {
    setSaving()
    const updated = await updateRole(id, roleId, patch)
    setMentee(updated)
    setSaved()
    return updated
  }
  ...
  function handleRoleEmergencyFlush(roleId, patch) {
    setSaving()
    return updateRole(id, roleId, patch, { keepalive: true })
      .then(updated => {
        setMentee(updated)
        setSaved()
        return updated
